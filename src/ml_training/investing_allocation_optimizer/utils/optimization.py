...
def bootstrap_portfolio_optuna_objective(
    ...
    Args:
        ...
        bootstrap_paths (Dict[str, pd.DataFrame]):
            ...
            bootstrap function. Keys are column names, values are
            DataFrames of size (n_periods x n_bootstrap_paths).
        custom_objective (Callable[[pd.DataFrame], float]):
            User-defined function that takes a weighted outcomes DataFrame
            and returns a scalar metric to maximize (e.g., expected return).
